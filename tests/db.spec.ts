...
describe("AppendOnlyClient", () => {
  ...
  describe("set method", () => {
    ...
    it("writes the key-value pair to the file", async () => {
      ...
      const content = fs
        .readFileSync(path.join(filename), "utf8")
        .split("\n")
        .filter(Boolean)
        .pop();
  ...
  describe("disconnect method", () => {
    it("closes the file descriptor", async () => {
      const client = new AppendOnlyKeyv(filename);

      await client.disconnect();

      const { fd } = client.opts;

      expect(() =>
        fs.writeSync(fd, Buffer.from("test"), 0, 4, 0),
      ).toThrowError();
    });

    it("throws if the file is deleted", async () => {
      const client = new AppendOnlyKeyv(filename);

      const { filePath } = client.opts;

      fs.unlinkSync(filePath);
      fs.closeSync(client.opts.fd);
