...
export class AppendOnlyKeyv extends EventEmitter implements KeyvStoreAdapter {
  ...
  async get<Value = unknown>(key: string): Promise<StoredData<Value> | undefined> {
    ...
    return new Promise<StoredData<Value> | undefined>((resolve, reject) => {
      fs.read(this.fd, buffer, 0, bytesRead, record.offset + kl, (err) => {
        ...
        const parsed: StoredData<Value> = JSON.parse(buffer.toString().trim());
        resolve((parsed as any).value.payload);
      });
    });
  }
  ...
  async set(key: string, value: string) {
    ...
    const bytesWritten = fs.writevSync(this.fd, [Buffer.from(serializedMessage)])
  ...
  private buildMessage(value: string | undefined, { deleted }: { deleted?: boolean } = {}): StoredData<Message> {
    return {
      value: {
        payload: value,
        timestamp: Date.now(),
